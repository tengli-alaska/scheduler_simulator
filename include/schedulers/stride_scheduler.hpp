...
public:
    ...
    void add_task(TaskPtr task, double /*current_time*/) override {
        ...
        task->set_stride(params_.stride_constant / weight);
...
private:
    ...
    void update_global_pass() {
        uint32_t min_pass = std::numeric_limits<uint32_t>::max();
        for (const auto& t : ready_queue()) {
            min_pass = std::min(min_pass, t->pass_value());
        }
        if (min_pass < std::numeric_limits<uint32_t>::max()) {
            global_pass_ = std::max(global_pass_, static_cast<uint64_t>(min_pass));
        }
    }
